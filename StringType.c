...
String *MakeString(char *str)
{
  ...
  if ( (string = (String *)AllocMem(sizeof(String), MEMF_PUBLIC)) )
  {
    ...
    if (string->length)
    {
      ...
      if ( (string->_guts = (char *)AllocMem(string->length + 1, MEMF_PUBLIC|MEMF_CLEAR)) )
      {
...
VOID StringTrimLeading(String *str)
{
  ...
  if (!str || !str->_guts || str->length < 1) return;
  ...
  for (srcIndex = 0; srcIndex < str->length;)
  {
    ...
    else
    {
      if (str->_guts[srcIndex] != ' ')
      {
        ...
        new_guts = (char *)AllocMem(str->length - srcIndex + 1, MEMF_PUBLIC|MEMF_CLEAR);
        destIndex = 0;
      }
      else
      {
...
VOID StringTrimTrailing(String *str)
{
  ...
  if (!str || !str->_guts ||
      str->length < 1 ||
      str->_guts[str->length - 1] != ' ') return;

  for (wsIndex = str->length;
       wsIndex > 0 && str->_guts[wsIndex - 1] == ' ';
       wsIndex--)
    ;
